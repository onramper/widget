...
const InputText: React.FC<InputTextType> = (props) => {
    ...
    return (
        <div className={`${styles.input} ${className}`}>
            {label && <label><span className={styles.label__title}>{label}{props.onHelpClick && <>&nbsp;&nbsp;<HintIcon onClick={props.onHelpClick} /></>}</span>{switchPairEnabled && symbols && !disabled && <SwitchPairs onClick={handleSwitchPair} pairs={symbols} indexSelectedNumber={actualSymbolIndex} />}</label>}

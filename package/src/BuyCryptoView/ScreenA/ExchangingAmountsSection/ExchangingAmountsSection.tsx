    ...
    const updateAmounts = () => {
      if (!collected.amountInCrypto) {
        return setAmounts({
          ...
          [ItemCategory.Crypto]: toStringOrDefault(collected.bestExpectedCrypto),
        });
      }

      return setAmounts({
        [ItemCategory.Currency]: toStringOrDefault(collected.bestExpectedCrypto),
        [ItemCategory.Crypto]: toStringOrDefault(collected.amount),
      });
    };

    updateAmounts();
  }, [collected.amount, collected.amountInCrypto, collected.bestExpectedCrypto]);
  ...
  useEffect(() => {
    const recalculateUnFocusedInput = () => {
      ...
      if (value !== 0 && unFocusedUnit) {
        value = Number(onChangeTextNumber(toStringOrDefault(value), unFocusedUnit?.precision));
      }
      handleInputChange("bestExpectedCrypto", value);
    };

    recalculateUnFocusedInput();
  }, [data.allRates, collected.amountInCrypto, handleInputChange, collected.selectedCrypto]);
      ...
      {[
        ...
        <InputDropdown
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            if (onChange(e, item.type, item.target?.precision) === false) return false;
          }}
          handleProps={{

...
const Skeleton = React.forwardRef<HTMLDivElement, { optionLeftStyle: React.CSSProperties }>((props, ref) => {
    ...
    return (<div className={`${styles.wrapper} ${commonStyles["skeleton-wrapper"]}`}>
        ...
        <div className={`${styles["option-info"]} ${commonStyles["skeleton-box"]}`} style={props.optionLeftStyle}>
...
const GatewayIndicator: React.FC<GatewayIndicatorProps> = (props: GatewayIndicatorProps) => {
    ...
    const adjustOptionLefWidth = () => {
        if(optionWrapperRef.current) {
            setOptionLeftStyle({maxWidth: `calc(100% - ${optionWrapperRef.current.scrollWidth + 10}px)`});
            return;
        }
        setOptionLeftStyle({});
    }
    ...
    if (props.isInitialLoading || !props.selectedGateway) return <Skeleton optionLeftStyle={optionLeftStyle} ref={optionWrapperRef} />;

...
export interface TransactionValidationParams extends ServiceQueryParams {
    amount: string;
    source: string;
    target: string;
    provider: string;
...
export class CurrencyNotFoundError extends CoreError {
    constructor(currencyId: string) {
        super(
            ...
            `The currency with Id: ${currencyId.trim()} cannot be found. Provide a valid currency Id.`
        );
    }
}
